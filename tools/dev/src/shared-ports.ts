...
export async function ensureSharedPortsResolved(
  ...
  if (runningWebUrl != null) {
    const runningWebPort = portFromUrl(runningWebUrl);
    if (runningWebPort != null) {
      options.webPort = String(runningWebPort);
      return;
    }
  }
...
export async function resolveSharedPortsFromRunningState(
  ...
  options: Pick<ToolDevOptions, "daemonPort" | "webPort">,
  urls: {
    readonly daemonUrl?: RunningUrlLookup;
    readonly webUrl?: RunningUrlLookup;
  },
): Promise<void> {
