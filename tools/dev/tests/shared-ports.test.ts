...
describe("tools-dev shared ports", () => {
  ...
  it("reserves an explicit web port before allocating a daemon port", async () => {
    const calls: Array<{ readonly label: string | undefined; readonly reserved: readonly number[] }> = [];
    const webPort = 45678;
    const daemonPort = 45679;
    const stubAllocate = async ({ label, reserved = new Set<number>() }: PortRequest = {}): Promise<PortAllocation> => {
