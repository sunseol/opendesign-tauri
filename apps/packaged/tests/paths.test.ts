...
describe("resolvePackagedNamespacePaths", () => {
  ...
  it("rejects unsafe namespace names", () => {
    const config = fakeConfig();
  ...
  it("does not read process.env implicitly so callers choose when to honor OD_DATA_DIR", () => {
    const config = fakeConfig();
  ...
  it("surfaces the relative-OD_DATA_DIR rejection as PackagedPathAccessError", () => {
    const config = fakeConfig();
