...
function designPreviewRank(file: ProjectFile, preferredName?: string | null): number {
  ...
  if (isProcessArtifactFileName(path)) return Number.POSITIVE_INFINITY;
  if (preferredName && path === preferredName) return 0;
  if (file.kind === 'html') return 10 + htmlPreviewNameRank(path);
  if (file.kind === 'image') return 30 + visualAssetNameRank(path);
