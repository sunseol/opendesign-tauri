...
export async function openExternalUrl(url: string): Promise<boolean> {
  ...
  if (openExternal != null) {
    try {
      ...
      // Fall through to daemon/current-tab fallback below.
    }
  }
  try {
...
export async function connectConnector(connectorId: string): Promise<ConnectorActionResult> {
  ...
  const openExternal = resolveDesktopBridge()?.openExternal;
  const useExternalBrowser = isOpenDesignHostAvailable() || typeof openExternal === 'function';
  try {
    ...
    if (json.auth?.kind === 'redirect_required' && json.auth.redirectUrl) {
      if (useExternalBrowser) {
        const opened = isOpenDesignHostAvailable()
          ? (await openHostExternalUrl(json.auth.redirectUrl)).ok
          : await openExternal!(json.auth.redirectUrl);
        if (!opened) {
          return {
