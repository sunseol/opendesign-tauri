...
const translate = (key: string, vars?: Record<string, string | number>) => {
  if (key === 'designFiles.showMore') {
    return `Show +${vars?.n ?? ''} more`;
  }
  return key;
};
...
function renderPane(extra: Partial<ComponentProps<typeof ChatPane>>) {
  return render(
    <ChatPane
      messages={[]}
      streaming={false}
...
describe('ChatPane imported folder artifacts', () => {
  ...
  it('shows the five most recently modified artifacts before revealing more', () => {
    ...
    expect(moreButton.textContent).toContain('Show +2 more');
