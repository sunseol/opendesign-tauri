...
describe('API proxy routes', () => {
  ...
  it('uses max_completion_tokens for GPT-5 OpenAI-compatible chat-completions payloads', async () => {
    const fetchMock = vi.fn((input: FetchInput, init?: FetchInit) => {
      const url = String(input);
      if (url.startsWith(baseUrl)) return realFetch(input, init);
      return Promise.resolve(sseResponse('data: [DONE]\n\n'));
    ...
    const body = JSON.parse(String(upstreamInit?.body)) as Record<string, unknown>;
  ...
  it('retries Azure deployment requests with max_completion_tokens when max_tokens is rejected', async () => {
    const fetchMock = vi.fn((input: FetchInput, init?: FetchInit) => {
      const url = String(input);
      if (url.startsWith(baseUrl)) return realFetch(input, init);
