...
describe('artifact runtime compatibility normalizer', () => {
  ...
  it('preserves non-HTML files and HTML that does not use Motion React hooks', () => {
    ...
    const vanillaMotionHtml =
      '<!doctype html><script src="https://unpkg.com/motion@11.11.13/dist/motion.js"></script><script>Motion.animate("div", { opacity: 1 })</script>';
  ...
  it('removes stale integrity from rewritten script tags', () => {
    ...
    const normalized = normalizeArtifactRuntimeImports('landing.html', html);
  ...
  it('aliases the FramerMotion global for framer-motion UMD artifacts', () => {
    ...
    expect(normalized.indexOf('framer-motion@11.11.17')).toBeLessThan(
      normalized.indexOf('window.FramerMotion'),
    );
  });
  ...
  it('normalizes content before writeProjectFile persists it', async () => {
    ...
    try {
      await writeProjectFile(
        ...
        'project-1',
        'landing.html',
        Buffer.from(brokenReactMotionHtml, 'utf8'),
      );
