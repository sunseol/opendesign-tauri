...
test('codex json stream surfaces disallowed connector tool selections as terminal errors', () => {
  ...
  handler.feed(
    JSON.stringify({
      ...
      item: {
        ...
        type: 'command_execution',
        command:
          'od tools connectors execute --connector github --tool github.github_list_notifications --input .daily-digest-tmp/notifications.json',
        aggregated_output: '',
        exit_code: null,
        status: 'in_progress',
      },
  ...
  assert.deepEqual(events, [
    ...
    },
    {
      type: 'error',
      message:
        'Connector tool github.github_list_notifications is not allowed for connector github. Re-list the connector catalog and choose one of the currently allowed read-only tools.',
    },
  ]);
});
...
test('codex json stream ignores unrelated connector tool-looking text', () => {
  const { events, handler } = collectEvents('codex');
  ...
  handler.feed(
    JSON.stringify({
      type: 'item.completed',
      item: {
        id: 'item-connector-text',
        type: 'command_execution',
        command: 'printf CONNECTOR_TOOL_NOT_FOUND',
        aggregated_output: 'CONNECTOR_TOOL_NOT_FOUND but not structured json',
        exit_code: 1,
        status: 'failed',
      },
    }) + '\n',
  ...
  assert.deepEqual(events, [
    ...
    {
      type: 'tool_result',
      toolUseId: 'item-connector-text',
      content: 'CONNECTOR_TOOL_NOT_FOUND but not structured json',
      isError: true,
    },
  ]);
});
