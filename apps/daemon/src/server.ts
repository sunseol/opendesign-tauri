...
export function resolveSafePromptImagePaths(imagePaths, opts = {}) {
  ...
  for (const inputPath of imagePaths) {
    ...
    if (!resolved.startsWith(uploadDir + pathImpl.sep) || !existsSync(resolved)) continue;
    ...
    async (req, res) => {
      try {
        ...
        if (req.file) {
          ...
          const desiredName = sanitizePath(
          ...
          if (def.id === 'amr' && event === 'error') {
            const failure = classifyAmrAccountFailure(
              [
                ...
                data?.error?.details === undefined ? '' : JSON.stringify(data.error.details),
                agentStdoutTail,
                agentStderrTail,
              ].join('\n'),
            );
