...
interface CollectBrowserUseDiscoveryFactsOptions {
...
interface BrowserUseRunStateInput {
  readonly requested: boolean;
  readonly agentId: string | null | undefined;
  readonly diagnostics?: BrowserUseDiscoveryFacts;
}
...
export function isBrowserUseRequested(...values: readonly unknown[]): boolean {
...
}: CollectBrowserUseDiscoveryFactsOptions = {}): BrowserUseDiscoveryFacts {
  try {
    ...
    return {
      ...
      probeFailureCategory:
        errorCode(error) === 'ENOENT' ? 'registry-missing' : 'registry-unreadable',
      staleThresholdMs,
    };
  }
}
...
export function buildBrowserUseRunState({
  ...
  agentId,
  diagnostics,
}: BrowserUseRunStateInput): BrowserUseRunState | null {
