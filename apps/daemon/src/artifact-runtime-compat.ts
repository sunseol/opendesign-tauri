...
const FRAMER_MOTION_ALIAS =
  '<script>window.FramerMotion = window.FramerMotion || window.Motion;</script>';
...
export function normalizeArtifactRuntimeImports(name: string, body: unknown): unknown {
  ...
  if (!MOTION_REACT_HOOK_USAGE.test(text) && !FRAMER_MOTION_GLOBAL_USAGE.test(text)) {
    return body;
  }
