...
export const SANDBOX_IMPORTED_PROJECT_UNAVAILABLE_MESSAGE =
  `Imported-folder projects are not available in ${SANDBOX_MODE_ENV} unless their root is under ${SANDBOX_IMPORT_ALLOWED_ROOTS_ENV}.`;
export const SANDBOX_IMPORT_ALLOWED_ROOTS_INVALID_MESSAGE =
  `${SANDBOX_IMPORT_ALLOWED_ROOTS_ENV} entries must be absolute paths.`;

export type SandboxRuntimeRoots = {
  ...
  readonly previewStateDir: string;
  readonly skillsCacheDir: string;
  readonly tempDir: string;
  readonly toolConfigDir: string;
};

export type SandboxRuntimeConfig = {
  readonly enabled: boolean;
  readonly dataDir: string;
  readonly roots: SandboxRuntimeRoots;
};

export class SandboxModeConfigError extends Error {
  constructor(message: string) {
    super(message);
    this.name = 'SandboxModeConfigError';
  }
}
...
export function isSandboxModeEnabled(
  ...
  throw new SandboxModeConfigError(
...
function configuredSandboxImportRoots(
  ...
  for (const entry of raw.split(path.delimiter)) {
    ...
    if (!path.isAbsolute(normalized)) {
      throw new SandboxModeConfigError(
        `${SANDBOX_IMPORT_ALLOWED_ROOTS_INVALID_MESSAGE} Got: ${trimmed}`,
      );
    }
...
export function resolveSandboxRuntimeConfigFromEnv(
  ...
  if (!rawDataDir) {
    throw new SandboxModeConfigError('OD_DATA_DIR is required when OD_SANDBOX_MODE is enabled');
  }
