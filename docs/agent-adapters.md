...
| **codebuddy** | `codebuddy` / `cbc` | Codebuddy CLI config | Codebuddy CLI managed | ❌ (prompt-injected) | ✅ | ✅ (`stream-json`, Claude-compatible) | P2 |
...
- Models: dynamic via the ACP handshake. If model discovery fails, the picker falls back to the CLI's default configuration rather than requiring startup detection to log in to Trae CLI.
