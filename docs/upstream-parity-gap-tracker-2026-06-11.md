...
| Design systems | `design-systems` 1,666 files | [#8](https://github.com/sunseol/opendesign-tauri/issues/8), [#34](https://github.com/sunseol/opendesign-tauri/issues/34), [#35](https://github.com/sunseol/opendesign-tauri/issues/35) | Complete - 150 official design-system 2.0 backfill outputs imported and guarded. #35 interaction parity is covered with shadcn registry import/API/CLI, Settings and `od` rename paths, user-system pin/order behavior, project picker switching, GitHub evidence/review flows, design-system new conversation actions, and manager filter/delete default-fallback coverage. |
| Plugins and marketplace | `plugins` 596 files | [#5](https://github.com/sunseol/opendesign-tauri/issues/5), [#21](https://github.com/sunseol/opendesign-tauri/issues/21), [#22](https://github.com/sunseol/opendesign-tauri/issues/22), [#47](https://github.com/sunseol/opendesign-tauri/issues/47) | In progress - Hallmark and the #4127 community slide/deck plugin pack are registered with folder-backed marketplace coverage, plugin context craft reads are normalized for manifest/apply snapshots, plugin-runtime frontmatter parsing preserves newline styles without accepting partial delimiters, and the official token-map atom now carries semantic inference guidance with a deterministic fixture guard |
...
- Design-system content is very large, but #34 now has catalog inventory counts
  and guard coverage for the migrated 150-package surface.
...
- Community plugin content now includes Hallmark plus the upstream #4127
  ...
  Plugin context craft helpers now normalize manifest craft requirements and
  preserve replay semantics from applied snapshots.
...
- AMR/BYOK/model-provider parity #4/#29 now carries project-screen BYOK
  ...
  Daemon provider model discovery now supports AIHubMix's public
  `/api/v1/models?type=llm` catalogue, filters media rows out of the chat
  picker, and preserves live system-proxy dispatchers for catalogue fetches.
...
- Governance #13 is complete: contributor-action links in `CONTRIBUTING.md`,
  `MAINTAINERS.md`, contributor-facing docs, and issue templates point at the
  Tauri fork, while a guard keeps fork identity and Tauri-specific PR/template
  evidence in place.
- Runtime helpers #50 now have a root guard for the eight helper package
  ...
  Daemon app-version metadata now infers `beta`, `preview`, and dotted
  `nightly` release channels from packaged app versions before falling back to
  development/stable defaults.
- Tools-dev #45 now loads workspace `.env.local` before resolving runtime
  ...
  split starts can preserve running ports without colliding. The `od` daemon
  URL resolver now falls back to `tools-dev status --json` after sidecar IPC,
  preserving live daemon discovery for default local runtimes. Upstream
...
- Shared UI package #44 now has the upstream `@open-design/components`
  ...
  harmonization and visual smoke coverage remain tracked under #9/#26/#44.
- Design-system catalog #34 now has 150 migrated project manifests, derived
  ...
  `od design-systems` help/rename/import-shadcn CLI coverage, Settings rename
  coverage, user-system pin/order behavior, project picker switching, web
  GitHub-evidence readiness, source-backed review cards, design-system project
  publish status, design-system new conversation actions, and manager
  filter/delete default-fallback coverage.
- Daemon runtime #7 now stages Grok Build prompts through daemon-owned temp
  ...
  The upstream HTTP route adapter scaffold is now present as a standalone
  `src/http` module with Express-boundary tests for parsing, shared API error
  envelopes, same-origin guards, handler exceptions, and dependency injection.
  ...
  literal list, runtime-def and media-execution API error codes, and reusable
  media policy denial/example payloads. Codex Browser Use requests now produce
  a typed unavailable run state, diagnostic event, prompt guard, spawn env, and
  diagnostics-export registry snapshot instead of silently permitting raw
  Chrome fallback. Cursor Agent stream parsing now emits timestamped partial
  chunks verbatim while reconciling `model_call_id` and non-timestamped
  terminal replays against the current turn, avoiding duplicate persisted
  output while still recovering a verified missing suffix.
  The final #33 plugin-manifest gap is closed by validating plugin-local
  manifest path fields before registry persistence: preview entries, local
  context assets, local skill/plugin refs, example outputs, compat paths, and
  custom GenUI component paths now reject absolute paths, traversal segments,
  URL-like schemes, and NUL bytes while preserving HTTPS preview media.
  Remaining sandbox resume, registry, and route parity stays tracked under
  #7/#48.
