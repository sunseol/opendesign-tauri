...
- Daemon runtime #7 now stages Grok Build prompts through daemon-owned temp
  ...
  media policy denial/example payloads. Cursor Agent stream parsing now emits
  timestamped partial chunks verbatim while reconciling `model_call_id` and
  non-timestamped terminal replays against the current turn, avoiding duplicate
  persisted output while still recovering a verified missing suffix.
