...
> **Status:** This is a docs-only guide. The flows below follow Alibaba Cloud's published product behaviour and the existing [`docs/deployment/docker.md`](../docker.md) and [`deploy/README.md`](../../../deploy/README.md) container model. Live ROS templates, provider-specific one-click scripts, and verification screenshots remain follow-up work for operators with active Alibaba Cloud accounts.
...
From this point the flow matches [`deploy/README.md`](../../../deploy/README.md):
...
This fork ships a baseline Helm chart under `charts/open-design`. The minimal manifest below remains useful when ACK operators need to inspect or adapt the raw Kubernetes shape before using Helm in production.
...
See [`deploy/README.md`](../../../deploy/README.md) for the full topology and reverse-proxy guidance.
...
This guide is intentionally docs-only. Follow-up contributions welcome:

- A first-party ROS template for one-click ECS provisioning.
- Live ACK verification for `charts/open-design`, including multi-replica safety guidance for external DB or shared volume topologies.
...
- Open Design one-click installer: [`deploy/README.md`](../../../deploy/README.md)
