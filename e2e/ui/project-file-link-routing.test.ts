...
test('[P1] assistant project file links open workspace tabs instead of new browser windows', async ({ page }) => {
  ...
  await expect(page.getByText('Finished single-file prototype:')).toBeVisible();
  ...
  await expect(
    page.frameLocator(ACTIVE_ARTIFACT_PREVIEW_SELECTOR).getByRole('heading', {
...
async function seedProjectWithAssistantFileLink(
  ...
  const localAbsoluteHref = '/Users/example/open-design/projects/File%20Link%20Routing/index.html';
  const assistantText = `Finished single-file prototype: [index.html](${localAbsoluteHref}).`;
...
async function waitForWorkspaceReady(page: Page): Promise<void> {
  await expect(page.getByText(/Loading Open Design/)).toHaveCount(0, { timeout: WORKSPACE_READY_TIMEOUT_MS });
  await expect(page.getByTestId('chat-composer')).toBeVisible({ timeout: WORKSPACE_READY_TIMEOUT_MS });
  await expect(page.getByTestId('file-workspace')).toBeVisible({ timeout: WORKSPACE_READY_TIMEOUT_MS });
}
