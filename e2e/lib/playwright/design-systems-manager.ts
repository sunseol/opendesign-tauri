...
export async function seedEntryBase(
  page: Page,
...
export async function routeDesignSystemsManager(
  page: Page,
  systems: UserSystem[],
  options: { readonly initialConfig?: Record<string, unknown> } = {},
): Promise<{
  readonly deletedDesignSystemIds: readonly string[];
  readonly persistedDesignSystemIds: readonly (string | null | undefined)[];
}> {
  const deletedDesignSystemIds: string[] = [];
  const persistedDesignSystemIds: Array<string | null | undefined> = [];
  let currentConfig = options.initialConfig ?? baseConfig();
  ...
  await page.route('**/api/**', async (route) => {
    ...
    if (/^\/api\/design-systems\/[^/]+$/.test(path) && method === 'DELETE') {
      ...
      deletedDesignSystemIds.push(id);
      await fulfillJson(route, { ok: index >= 0 });
      return;
    }
    if (path === '/api/skills') {
  ...
  return { deletedDesignSystemIds, persistedDesignSystemIds };
}
