...
test("design-system project manifest schema accepts shadcn source metadata", () => {
  const result = validateDesignSystemProjectManifest({
    schemaVersion: DESIGN_SYSTEM_PROJECT_SCHEMA_VERSION,
    id: "shadcn-button",
    name: "shadcn Button",
    ...
    source: {
      type: "shadcn",
      reference: "button",
      registryUrl: "https://ui.shadcn.com/r/styles/default/button.json",
      item: "button",
  ...
  assert.equal(result.ok, true);
  if (result.ok) {
    assert.equal(result.manifest.source.type, "shadcn");
  }
});
