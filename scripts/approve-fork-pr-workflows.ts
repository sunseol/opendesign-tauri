...
export async function listPendingApprovalRuns(
  ...
  const loadWorkflowRunsResponsePage =
    deps.loadWorkflowRunsResponsePage ?? ((path: string) => github<WorkflowRunsResponse>(path));
  ...
  const loadPullRequestsForHeadRef =
    deps.loadPullRequestsForHeadRef ??
    ((currentRepo: string, headOwnerAndRef: string) => listPullRequestsForHeadRef(currentRepo, headOwnerAndRef));
